    ...
    for i, row in data.iterrows():
        ...
        if trading_signal:
            if capital > row.Close * n_shares * (1 + COM):
                capital -= row.Close * n_shares * (1 + COM)
                active_positions.append(Position(ticker="AAPL", price=row.Close,  n_shares = n_shares, timestamp = row.Timestamp))
        ...
        if trading_signal_short:
            ...
            if capital >= required_margin:
                ...
                short_positions.append(Position(ticker="AAPL", price=row.Close, n_shares=n_shares, timestamp=row.Timestamp))

        # Portfolio value
        long = sum([position.n_shares * row.Close for position in active_positions]) # Long value is based on last close price
...
def backtestP(indicators: list, data: pd.DataFrame, sl: float, tp: float,
              rsi_window: int = 0, rsi_lower: int = 0, rsi_upper: int = 0,
              boll_wind: int = 0, boll_wind_dev :int = 0,
              window_slo_macd :int = 0, window_fast_macd :int = 0, win_sig_macd :int = 0,
              adx_window :int = 1 ,short_sma_wind:int =0, long_sma_wind :int =0,
